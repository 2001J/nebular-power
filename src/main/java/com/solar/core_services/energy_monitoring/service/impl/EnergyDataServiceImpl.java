...
public class EnergyDataServiceImpl implements EnergyDataService {
    ...
    @Override
    public List<EnergyDataDTO> getReadingsInDateRange(Long installationId, LocalDateTime startDate, LocalDateTime endDate) {
        // Verify the installation exists
        SolarInstallation installation = installationRepository.findById(installationId)
                .orElseThrow(() -> new ResourceNotFoundException("Solar installation not found with ID: " + installationId));
    ...
    public DashboardResponse getDashboardData(Long customerId) {
        ...
        // In a real application, you might want to aggregate data from all installations
    ...
    public DashboardResponse getInstallationDashboard(Long installationId) {
        ...
        double currentPowerConsumption = recentReadings.isEmpty() ? 0 : recentReadings.get(0).getPowerConsumptionWatts();
        ...
        Double monthToDateGeneration = energyDataRepository.sumPowerGenerationForPeriod(installation, startOfMonth, endOfDay);
        Double monthToDateConsumption = energyDataRepository.sumPowerConsumptionForPeriod(installation, startOfMonth, endOfDay);
        ...
        double monthToDateConsumptionKWh = (monthToDateConsumption != null ? monthToDateConsumption : 0) / 1000.0 / 3600.0;
