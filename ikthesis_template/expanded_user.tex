...
The Solar Energy Monitoring and Control System enables real-time monitoring of solar installations and automated payment tracking for Pay-As-You-Go (PAYG) financing models. Key capabilities: energy monitoring, payment management, security monitoring, role-based access, and reporting analytics.
...
Main components: Web Dashboard, Device Simulation (Raspberry Pi-based), and Notification System. The system implements role-based access control (Table~\ref{tab:role-privileges}).
...
Server: Dual-core 2.5GHz CPU (quad-core 3.0GHz recommended), 8GB RAM (16GB recommended), 100GB storage (500GB recommended), 10 Mbps network. Software: Ubuntu 20.04+/CentOS 8/Windows Server 2019/macOS 10.15+, Java 21, PostgreSQL 14+, SSL certificate.
...
\textbf{Customers:} Provider creates account; activate via email link; set password (8+ characters with uppercase, lowercase, number, special character).

\textbf{Administrators:} Set up during deployment; change default password on first login.
...
\textbf{Browser:} Chrome 90+, Firefox 88+, Edge 90+, Safari 14+, Opera 74+. JavaScript and cookies enabled, 1280$\times$768 minimum resolution.

\textbf{Network:} 1 Mbps minimum (5 Mbps recommended), HTTPS required.

\textbf{Simulation (Optional):} Raspberry Pi 3B+/4, Python 3.8+, packages: requests, numpy, schedule, paho-mqtt.

\textbf{Security:} Ports 443 (HTTPS), 80 (HTTP redirect), 5432 (PostgreSQL), 22 (SSH). TLS 1.2+ required.
...
Docker Desktop 4.0+/Docker Engine 20.10+, Docker Compose 2.0+, 4GB RAM (8GB recommended), ports 3000, 8080, 5432 available.
...
Edit \texttt{.env} (copied from \texttt{.env.template}): set \texttt{ADMIN\_EMAIL}, \texttt{ADMIN\_PASSWORD}, \texttt{JWT\_SECRET}. Defaults for Postgres are provided and can be changed if needed (\texttt{DB\_*}).
...
This starts three containers: \texttt{postgres}, \texttt{solar\_backend}, \texttt{solar\_frontend}. Wait until \texttt{postgres} is healthy.

\textbf{3) Access the app}
\begin{itemize}
    \item Frontend: \url{http://localhost:3000}
    \item Backend API (Swagger UI): \url{http://localhost:8080/swagger-ui.html}
\end{itemize}

Log in with the admin you set in \texttt{.env} (defaults: \texttt{admin@solar.com} / \texttt{admin123}); change the password on first login.

\subsection{Configuration}
\label{subsec:system-configuration}

Configuration is centralized in \texttt{.env} and consumed by \texttt{compose.yaml}:
\begin{itemize}
    \item \textbf{Database}: \texttt{DB\_HOST}, \texttt{DB\_PORT}, \texttt{DB\_NAME}, \texttt{DB\_USER}, \texttt{DB\_PASSWORD} (backend uses JDBC URL \texttt{jdbc:postgresql://\$\{DB\_HOST\}:\$\{DB\_PORT\}/\$\{DB\_NAME\}})
    \item \textbf{Auth}: \texttt{JWT\_SECRET}, \texttt{JWT\_EXPIRATION\_MS}
    \item \textbf{URLs}: \texttt{FRONTEND\_URL} (used in backend links), \texttt{BACKEND\_URL}
    \item \textbf{Mail (optional locally)}: \texttt{MAIL\_HOST}, \texttt{MAIL\_PORT}, \texttt{MAIL\_USERNAME}, \texttt{MAIL\_PASSWORD}
    \item \textbf{Admin bootstrap}: \texttt{ADMIN\_EMAIL}, \texttt{ADMIN\_PASSWORD}, \texttt{ADMIN\_FULL\_NAME}
    \item \textbf{Profile}: \texttt{SPRING\_PROFILES\_ACTIVE=prod} (uses PostgreSQL per \texttt{application-prod.properties})
\end{itemize}

For production, set strong secrets (JWT, DB password), real mail credentials, enable HTTPS at your reverse proxy, and configure CORS to allow only your domains.

\subsection{Device Simulation (Optional)}
\label{subsec:device-simulation}

\textbf{Local simulator (Raspberry Pi or any Python 3.8+ host)}
\begin{verbatim}
pip3 install -r pi_simulation/requirements.txt
# edit pi_simulation/config.json -> set backend base URL (e.g., http://localhost:8080)
python3 pi_simulation/main.py
\end{verbatim}

\subsection{Verification and Management}
\label{subsec:verification}

\textbf{Verify deployment}
\begin{itemize}
    \item Container health: \texttt{docker compose ps}
    \item Logs: \texttt{docker compose logs -f backend} (and \texttt{postgres}, \texttt{frontend} as needed)
    \item UI check: Open \url{http://localhost:3000} and sign in with admin credentials
\end{itemize}

\textbf{Common issues}
\begin{description}
    \item[Port conflicts] Change \texttt{POSTGRES\_HOST\_PORT} or host ports in \texttt{compose.yaml}
    \item[Database init delay] Wait 2--3 minutes for Postgres; check \texttt{docker compose logs postgres}
    \item[Backend DB auth] Verify \texttt{DB\_*} in \texttt{.env} match Postgres env
\end{description}
...
\begin{itemize}
    ...
    \item Restart service: \texttt{docker compose restart backend}
    \item Tail logs: \texttt{docker compose logs -f backend}
\end{itemize}

\subsection{Account Setup and Access}
\label{subsec:account-setup}

\textbf{Customer Onboarding:} Admin creates account via Customer Management $\rightarrow$ Add Customer. Customer receives activation email and sets password (8+ characters, mixed case, number, special character). See Figure~\ref{fig:customer-onboarding}.
